...
use aidoc_model::{
    Change, ChangeType, HashRef, Node, Operation, OperationType, Provenance, Revision,
};
...
pub fn branch_head(store: &Store, doc_id: &str, name: &str) -> Result<String, BranchError> {
    ...
    let found = if name == "main" {
        store
            .conn()
            .query_row(sql, [doc_id], |r| r.get(0))
            .optional()?
    } else {
        store
            .conn()
            .query_row(sql, rusqlite::params![doc_id, name], |r| r.get(0))
            .optional()?
    };
...
fn snapshot(store: &Store, doc_id: &str, rev: &str) -> Result<Vec<Node>, BranchError> {
    if let Some(nodes) = crud::load_snapshot(store.conn(), doc_id, rev)? {
        return Ok(nodes);
    }
    ...
    if rev == "R000" {
        ...
        if let Some(child) = child {
            if let Some(nodes) = crud::load_snapshot(store.conn(), doc_id, &child)? {
                return Ok(nodes);
            }
        }
    }
...
pub fn merge_branch(
...
) -> Result<RevisionId, BranchError> {
    if name == "main" || name.trim().is_empty() {
        return Err(BranchError::NotFound(name.into()));
    }
    if crud::head_branch(store.conn(), doc_id)?.is_some() {
        return Err(BranchError::NotOnMain);
    }
    let main = crud::head_revision(store.conn(), doc_id)?
        .ok_or_else(|| BranchError::NotFound("main".into()))?;
    let source = branch_head(store, doc_id, name)?;
    let main_ancestors = ancestors(store, doc_id, &main)?;
    let mut cursor = source.clone();
    let base = loop {
        if main_ancestors.contains(&cursor) {
            break cursor;
        }
        cursor = crud::list_parents(store.conn(), doc_id, &cursor)?
            .into_iter()
            .next()
    ...
    if !conflicts.is_empty() {
        return Err(BranchError::Conflicts(conflicts));
    }
    // A merged tree must not strand children under a deleted parent.
    for node in merged.values() {
        if let Some(parent) = &node.parent {
            if !merged.contains_key(parent.as_str()) {
    ...
    let op = Operation {
        ...
        target: None,
        expected_revision: RevisionId::new(main.clone()),
        expected_hash: None,
        target_revision: Some(RevisionId::new(source.clone())),
        targets: vec![],
    ...
    store.tx(|tx| {
        ...
        crud::insert_revision(
            ...
            &Revision {
                id: revision.clone(),
                parent: Some(RevisionId::new(main.clone())),
        ...
        tx.execute(
            "INSERT INTO revision_parents(doc_id,revision,parent,seq) VALUES(?1,?2,?3,1)",
            rusqlite::params![doc_id, revision.as_str(), source],
        )?;
        crud::advance_head(tx, doc_id, revision.as_str())?;
        for id in &changed {
            let before = main_map.get(id).map(|n| HashRef {
                hash: sha256_hex(n.content.as_bytes()),
            });
            let after = merged.get(id).map(|n| HashRef {
                hash: sha256_hex(n.content.as_bytes()),
            });
            crud::insert_change(
                tx,
                doc_id,
                &Change {
                    id: format!("CH-{}-{id}", revision.as_str()),
                    revision: revision.clone(),
                    node: NodeId::from_validated(id.clone()),
                    change_type: ChangeType::Merge,
                    before,
                    after,
                    summary: Some(format!("merged from {name}")),
                },
            )?;
        }

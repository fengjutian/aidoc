...
pub fn check(store: &Store, doc_id: &str, report: &mut ValidationReport) -> Result<(), IdentityError> {
    ...
    for n in &nodes {
        if !seen.insert(n.id.clone()) {
            report.identity_errors.push(format!("duplicate node id: {}", n.id.as_str()));
        }
        if n.id.as_str() == "root" {
            has_root = true;
        }
        if let Err(e) = aidoc_model::id::NodeId::new(n.id.as_str()) {
            report.identity_errors.push(format!("invalid node id: {} ({})", n.id.as_str(), e));
        }
    }

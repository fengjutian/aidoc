...
pub fn check(store: &Store, doc_id: &str, report: &mut ValidationReport) -> Result<(), CodeRefError> {
    let nodes = crud::list_nodes(store.conn(), doc_id)?;
    for n in nodes {
        if matches!(n.kind, aidoc_model::NodeKind::CodeRef) {
            if n.attributes.get("file").map(|s| s.is_empty()).unwrap_or(true) {
                report.code_ref_errors.push(format!(
                    "code-ref {} missing file attribute",
                    n.id.as_str()
                ));
            }
        }
